...
import TuneBank.Data.CommentId (CommentKey, CommentId)
  ...
  handleAction = case _ of
            ...
            Just key -> do
              ...
              case eComment of
                Right comment -> do
                  ...
                  H.modify_ (\st -> st { submission = submission } )
                  pure unit
                Left _ -> do
                  -- new comment
                  H.put newState
    ...
    SubmitComment event -> do
      ...
      case state.currentUser of
        ...
        Just credentials -> do
          baseURL <- getBaseURL
          -- TO DO - commentid is not generated by the client any more           
          let
            rawSubmission =
              case state.key of
                Nothing ->
                  -- new comment
                  state.submission
                    { subject = state.submission.subject
